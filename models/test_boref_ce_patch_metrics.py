"""
test_train_boref_ce_patch.py

Test script for patch-based training of BoRefAttnNet (CE-only).
"""
...
CHECKPOINT_PATH = "boref_ce_patch_checkpoints/epoch_74.pth"
...
# Initialize validation dataset and loader for quick test
    ...
    with torch.no_grad():
        for idx, sample in enumerate(val_loader):
            ...
            for (d0, h0, w0), patch_img in sliding_window_subvolumes(img, PATCH_SIZE, STRIDE):
                ...
                seg_logits, attention_maps = model(patch_img)

                print(f"Sample[{idx}] Patch[{patches_processed}] logits shape: {seg_logits.shape}")

                patches_processed += 1

                # For test purposes, limit to 2 patches per subject
